...
public class BlockBreakRequirement extends ProgressiveRequirement {
  ...
  public String buildRemainingString(Player player) {
    ...
    if (remaining == 0) {
      return "<st><dark_gray>Romper x" + ((int) getTotal(player)) + " <lang:" + material.translationKey() + ">:</dark_gray></st> <#80ff00>¡Completado!</#80ff00>";
    } else {
      return "<#adadad>Romper <#ffb000>x" + ((int) getTotal(player)) + "</#ffb000> <#ffec00><lang:" + material.translationKey() + "></#ffec00>:</#adadad> <#ff4444>" + ((int) remaining) +" restantes</#ff4444>";
    }
  }
}
